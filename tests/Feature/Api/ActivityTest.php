...
test('get activities without permission', function () {
...
test('paginate activities', function () {
    $user = getUser();
...
test('paginate activities with search param', function () {
    $user = getUser();
...
test('paginate activities with more records per page than allowed', function () {
    $this->actingAs(getUser(), self::API_GUARD)
        ->getJson('/api/activity?per_page=140')
        ->assertStatus(422);
});

test('paginate activities for user', function () {
